...
const DigitInputGroup = observer(
  ({ numDigits, ...props }: FlexProps & DigitInputGroupProps) => {
    ...
    const keyDownListener = action((e: KeyboardEvent) => {
      ...
      if (!isNaN(parseInt(e.key))) {
        digits[nextEmptyIndex == -1 ? digits.length - 1 : nextEmptyIndex] =
          e.key;
    ...
    return (
      <Flex direction="row" {...props}>
        {digits.map((_, idx) => (
          <DigitInput key={idx} value={digits[idx]} />
        ))}
      </Flex>
    );
  }
...
const BackArrow = () => (
  ...
  >
    ...
    />
    <Text color="petcode.neutral.400" fontSize="2xl" marginLeft={4}>
      Go Back
    </Text>
  </Link>
);
...
const HeaderTextGroup = () => (
  <Box marginX={4}>
    ...
    <Text
      ...
      maxWidth="700px"
    >
      Enter the 4 digit recovery code that was sent to the email linked with
      your account.
    </Text>
  </Box>
);
